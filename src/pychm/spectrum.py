...
def _higgs_mass2(P, xi, route, f):
    """m_h^2 = (1-xi) V''(s_h) / f^2 at the minimum (V'' = d^2 V/d s_h^2)."""
    sh = np.sqrt(xi)
    h = 1e-3
    V = lambda s: potential.routes.potential_curve(P, np.array([0.0, s]), route=route)[1]
    Vpp = (V(sh + h) - 2*V(sh) + V(sh - h))/h**2
    return (1 - xi)*Vpp/f**2


def spectrum(P, route='eigenvalue'):
    ...
    f = V_EW/np.sqrt(xi)
    sh = np.sqrt(xi)
    mU = np.sort(np.linalg.svd(mchm5.mass_U(P, sh), compute_uv=False))   # u,c,t,partners
    mD = np.sort(np.linalg.svd(mchm5.mass_D(P, sh), compute_uv=False))   # d,s,b,partners
    mh2 = _higgs_mass2(P, xi, route, f)
    return dict(
        xi=xi, f=f,
