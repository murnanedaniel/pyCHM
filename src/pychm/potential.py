...
def potential_coeffs(P, route='eigenvalue', sh_max=0.32, n=13):
...
def _Vfun(P, route):
    ...
    V0 = routes.potential_curve(P, np.array([0.0, 1e-6]), route=route)[0]
    def V(sh):
        return routes.potential_curve(P, np.array([0.0, abs(sh)]), route=route)[1]
    return V


def vacuum_xi(P, route='eigenvalue', sh_hi=0.7):
