...
\impl{groups.so5.gen\_vector, groups/so5.py}{test\_so4\_unbroken\_generators\_close\_the\_algebra}
...
matrix.''
\impl{groups.so5.U\_vector, so5.U\_vector\_sym}{test\_U\_vector\_matches\_thesis\_goldstone}
...
\impl{tensors.tensor\_basis, tensors.U\_rep\_tensor, groups.so5.U\_rep}{test\_tensors.py (dimension, unitarity, homomorphism)}
...
\impl{groups/derive.py::solve\_composite}{test\_dressings\_are\_matrix\_elements}
...
\impl{so5.overlap\_sym, mchm5.formfactor\_pieces}{test\_coeffs\_5\_5\_5}
...
\impl{decompose.channel\_weights\_sym, so5.embedding\_sym}{test\_14\_channel\_weights\_derive\_thesis\_prefactors, test\_4\_5\_is\_derived\_from\_the\_embedding}
...
\section{Beyond SO(N): the coset abstraction and SU(4)/Sp(4)}
\label{sec:coset}
Everything above is built from one group-agnostic engine (\code{pychm.groups}): a coset $G/H$ is a
list of unbroken generators of $H$ and broken generators $X^a$ in some representation, and the
Goldstone is $U(\pi)=\exp(i\,\pi^a X^a)$ -- in unitary gauge a rank-$\le2$ rotation, so the closed
Rodrigues form \eqref{eq:U6} serves every case. The MCHM and NMCHM are the instances
$\SO5/\SO4$ and $\SO6/\SO5$; the \emph{same code} builds $\SU N/\Sp N$ cosets, with the SU($N$)
Gell-Mann generators and the USp($N$) subalgebra obtained from the Cartan involution
$\theta(X)=\Omega\,\bar X\,\Omega^{-1}$ ($\Omega$ the symplectic form): $\mathfrak{usp}(N)$ is the
$\theta=-1$ eigenspace, the coset the $\theta=+1$ eigenspace.

\paragraph{SU(4)/Sp(4) $\cong$ SO(6)/SO(5).}
The minimal pseudoreal Ferretti--Sannino coset $\SU4/\Sp4$ has $15-10=5$ broken generators --- the
same five pNGBs (Higgs doublet $+$ singlet) as $\SO6/\SO5$. Under $\SU4\cong\mathrm{Spin}(6)$,
$\Sp4\cong\mathrm{Spin}(5)$ the representations map as
\begin{equation}
  \mathbf 4_{\SU4}=\mathbf 4_{\text{spinor}}^{\SO6},\quad
  \mathbf 6_{\SU4}^{[\text{antisym}]}=\mathbf 6_{\text{vector}}^{\SO6},\quad
  \mathbf{15}_{\SU4}=\mathbf{15}^{\SO6}.
\end{equation}
The library builds $\SU4/\Sp4$ independently (its own generators, Goldstone, and the antisymmetric
$\mathbf 6=[\mathbf4\otimes\mathbf4]_A$) and cross-checks it against the existing $\SO6$ code, which
is an \emph{exact oracle}: the $\mathbf4$ branches to a single $\mathbf4$ of $\Sp4$ and the
$\mathbf6$ to $\mathbf1\oplus\mathbf5$, matching the $\SO6$ spinor and vector; and the strong
statement of the isomorphism --- the $\mathbf6$ generators span the full $15$-dimensional
$\so6\cong\mathfrak{su}(4)$ algebra and act irreducibly (commutant $=$ scalars, by Schur) --- holds,
so the SU(4) antisymmetric $\mathbf6$ \emph{is} the irreducible SO(6) vector. The new coset thus
reproduces the NMCHM group theory while sharing all of its tooling.
\impl{groups/lie.py (su\_generators, sp\_subalgebra), groups/coset.py, groups/su4sp4.py}{test\_su4sp4.py}

\section{SU(5)/SO(5) (littlest Higgs) and the coset landscape}
\label{sec:su5}
$\SU5/\SO5$ is a type-AI symmetric space: the Cartan involution $\theta(X)=\bar X$ (complex
conjugation) splits $\mathfrak{su}(5)$ into the unbroken $\so5$ (10 imaginary-antisymmetric
generators, the $\theta=-1$ eigenspace) and the coset (the $\theta=+1$ eigenspace, 14 real-symmetric
generators). The same engine that built the SU(4)/Sp(4) split builds this one
(\code{lie.cartan\_split} with the conjugation involution). The 14 pNGBs are the symmetric-traceless
\textbf{14} of $\SO5$, which branches under custodial $\SO4$ as
\begin{equation}
  \mathbf{14}\;=\;(3,3)\oplus(2,2)\oplus(1,1)\;=\;\text{complex triplet }\phi\;\oplus\;\text{Higgs doublet }h\;\oplus\;\text{singlet }\eta,
\end{equation}
the littlest-Higgs pNGB content. The Goldstone in the fundamental \textbf{5} is $U=\exp(i\,\theta^a
X^a)$ over the coset generators (a complex unitary rotation, unlike the real SO(N) one); along the
Higgs axis the $(2,2)$ overlap magnitude reduces to the MCHM value
$|\langle q_L|U|t_R\rangle|^2=\sh^2/2$, so the top and Higgs match the 5-5-5 there. The symmetric
\textbf{15} of SU(5) (the fuller partner rep) is built with the SU(N) tensor basis (no
$\delta$-trace removal: \code{tensor\_basis(\dots,group='SU')}) and branches $\mathbf{15}=\mathbf{14}
\oplus\mathbf 1$ under $\SO5$. The genuinely new observables are the singlet and triplet pNGB masses,
the curvatures of the two-extra-field Coleman--Weinberg potential at the vacuum (even in $\eta,\phi$,
so $\langle\eta\rangle=\langle\phi\rangle=0$).
\impl{groups/lie.py (cartan\_split), groups/su5so5.py, su5so5.py}{test\_su5so5.py}

\paragraph{The landscape.} A composite Higgs needs a custodial $\SO4\cong\SU2_L\times\SU2_R\subset H$
and a $(2,2)$ Higgs in the coset $G/H$. Chala \& Fonseca (arXiv:2309.10635) enumerate all compact
$G/H$ with $\le13$ pNGBs satisfying this (642 models; complete up to 13 NGBs). \code{groups.landscape}
reproduces a slice from pyCHM's own \code{Coset}: it branches each coset under the custodial $\SO4$
and flags the $(2,2)$, recovering MCHM $(2,2)$, NMCHM $(1,1)+(2,2)$, and littlest-Higgs
$(1,1)+(2,2)+(3,3)$, and discovering $\SO7/\SO6$, $\SO8/\SO7$ (see \code{docs/COSET\_LANDSCAPE.md}).
\impl{groups/landscape.py}{test\_landscape.py}
